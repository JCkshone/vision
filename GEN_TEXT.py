...
class GenText:
    ...
    @classmethod
    def get_annot_rect(cls, annot):
        return np.array(list(map(lambda x: int(x), annot[3:])))

    @classmethod
    def iou(cls, obj_proposal, annot_rect):
    ...
    @classmethod
    def get_bg_proposals(cls, object_proposals, annot):
        annot_rect = cls.get_annot_rect(annot)
        bg_proposals = []
        for obj_proposal in object_proposals:
            if cls.iou(obj_proposal, annot_rect) <= 0.5:
                bg_proposals.append(obj_proposal)
        return bg_proposals

    @classmethod
    def get_object_proposals(cls, img, scale=500, sigma=0.9, min_size=10):
        # Selective search
    ...
    @classmethod
    def gen_annot_file_line(cls, img_fn, class_name, train_subset_class, rect):
        rect = ' '.join(map(str, rect))
        line = ' '.join([img_fn, class_name, train_subset_class, rect])
        return line

    @classmethod
    def gen_annot_file_lines(cls, annot):
        lines = []

        # Get original annot line
        img_fn, class_name, train_subset_class = cls.parse_annot(annot)
        annot_rect = cls.get_annot_rect(annot)
        lines.append(cls.gen_annot_file_line(img_fn, class_name, train_subset_class, annot_rect))

        # Load image
        img = skimage.io.imread(os.path.join(TRAIN_IMAGE_DIR, img_fn))

        # Selective search
        object_proposals = cls.get_object_proposals(img)
        if len(object_proposals) == 0:
            return lines

        # Background proposals
        bg_proposals = cls.get_bg_proposals(object_proposals, annot)
        ...
        x1, y1, x2, y2 = bg_proposal[0], bg_proposal[1], bg_proposal[0] + bg_proposal[2], bg_proposal[1] + bg_proposal[
            3]
        lines.append(cls.gen_annot_file_line(img_fn, CLASS_NAME[-1], train_subset_class, [x1, y1, x2, y2]))
    ...
    def process(cls):
        ...
        with ProcessPoolExecutor(n_workers) as executer, open(ANNOT_FILE_WITH_BG, 'w') as fw:
            for annot in annot_train:
                results.append(executer.submit(cls.gen_annot_file_lines, annot))
