...
class ImgDetect:
    ...
    def parse_cmdline(cls):
        ...
        return parser.parse_args(['img_fn'])

    @classmethod
    def scaling(cls, image_data):
        return (image_data.astype(np.float32) - (PIXEL_DEPTH / 2)) / PIXEL_DEPTH

    @classmethod
    def iou_xywh(cls, box1, box2):
    ...
    @classmethod
    def nms(cls, recog_results, pred_prob_th=0.99, iou_th=0.5):
        # nms results
        nms_results = []
        ...
        cand_nms_idx = cls.update_idx(cand_results)
        ...
        while len(cand_results) > 0:
            ...
            for seq_i, i in enumerate(cand_nms_idx):
                if cls.iou_xywh(cand_results[i]['obj_proposal'], pred['obj_proposal']) >= iou_th:
                    del_idx.append(i)
    ...
    @classmethod
    def logo_recognition(cls, sess, img, obj_proposal, graph_params):
        # recognition results
        recog_results = {}
    ...
    def setup_graph(cls):
        ...
        with graph_params['graph'].as_default():
            ...
            graph_params['target_image'] = tf.placeholder(tf.float32, shape=(
            1, MODEL.CNN_IN_HEIGHT, MODEL.CNN_IN_WIDTH, MODEL.CNN_IN_CH))
    ...
    @classmethod
    def process(cls):
        #args = cls.parse_cmdline()
        img_fn = os.path.abspath("img_fn")
        ...
        # Load target image
        target_image = cls.load_target_image(img_fn)

        # Get object proposals
        object_proposals = cls.get_object_proposals(target_image)

        # Setup computation graph
        graph_params = cls.setup_graph()
        ...
        for obj_proposal in object_proposals:
            ...
            results.append(cls.logo_recognition(sess, crop_image, obj_proposal, graph_params))
        ...
        nms_results = cls.nms(results, PRED_PROB_THRESH, iou_th=0.7)
